...
#        gru_state = self.apply_rnn(self.input_sequence, self.input_sequence_lengths) 
        ...
        layer1_para = tf.nn.tanh(linear('sm_layer1_para', gru_state, [self.config.hidden_size, self.config.layer1_size]))
        layer2_para = tf.nn.tanh(linear('sm_layer2_para', layer1_para, [self.config.layer1_size, self.config.layer2_size]))
...
#            self.score_layer = mixing_w*self.layer4_para + (1-mixing_w)*tf.transpose(tf.sparse_tensor_dense_matmul(ancestry_sparse_tensor, tf.transpose(self.layer4)))
